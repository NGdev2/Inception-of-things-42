...
if ! command -v docker &> /dev/null; then
  ...
  echo -e "${YELLOW}⚠️  You may need to log out and back in for Docker group changes to take effect${RESET}"
else
...
if k3d cluster list | grep -q "$CLUSTER_NAME"; then
  ...
  echo -e "${GREEN}🏗️  Creating K3d cluster named '$CLUSTER_NAME'...${RESET}"
  k3d cluster create $CLUSTER_NAME
fi

# Export KUBECONFIG
echo -e "${GREEN}⚙️  Exporting KUBECONFIG for current session...${RESET}"
...
echo -e "${BLUE}export KUBECONFIG=\$(k3d kubeconfig write $CLUSTER_NAME)${RESET}"
...
echo -e "  • K3d cluster: $CLUSTER_NAME"
echo -e "  • Namespaces: argocd, dev"
echo -e "  • ArgoCD: Installing (pods starting up)"
echo ""
echo -e "${YELLOW}📝 Next Steps:${RESET}"
echo -e "  1. Wait for ArgoCD to be ready: kubectl get pods -n argocd"
echo -e "  2. Initialize ArgoCD: ./argocd_init.sh"
echo -e "  3. Setup GitLab: ./start_gitlab_ce_docker.sh"
echo ""
echo -e "${BLUE}🔍 Monitor ArgoCD startup:${RESET}"
echo -e "  kubectl get pods -n argocd -w"