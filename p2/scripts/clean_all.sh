...
if vagrant status | grep -q "running"; then
    ...
    vagrant ssh -c "
        ...
        # Delete all pods, services, ingress in default namespace
        kubectl delete pods,services,ingress --all
...
if VBoxManage list vms | grep -q "$VM_NAME"; then
    ...
    VBoxManage controlvm "$VM_UUID" poweroff 2>/dev/null || true
...
echo "✓ K3s cluster and all pods/services/ingress"
